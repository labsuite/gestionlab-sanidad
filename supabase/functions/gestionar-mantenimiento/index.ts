...
Deno.serve(async (req) => {
  ...
  if (accion === "registrar" || accion === "finalizar" ||
    ...
    // El alumnado firma los mantenimientos con el nombre de su grupo ("1º CS LCB"),
    // nunca con un nombre propio: quien da validez al registro es el docente que
    // pone el visto bueno (supervisado_por). Ver docs/proteccion-datos.md.
    const firmaGrupo = firmaAlumnado(email, nombreUsuario);
    ...
    // El alumnado firma siempre con su grupo (nunca con el nombre que venga del
    ...
    const comun: Record<string, unknown> = {
      ...
      realizado_por: esStaff ? realizadoPor : firmaGrupo,
