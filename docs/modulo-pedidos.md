...
1. Descarga el archivo de Storage y lo manda a `gemini-1.5-flash` con `responseSchema` (structured output) pidiendo un array de líneas `{material, cantidad, precio_unitario, unidad}` — **`precio_unitario` es explícitamente el precio sin IVA** (coherente con `lineas_pedido.precio_unitario` y con el aviso de `modal-precios`: "Precios unitarios introducidos sin IVA"). El prompt instruye a ignorar IVA/totales/cabeceras y a devolver aparte, si existe, un cargo de transporte/portes como `cargo_extra: {concepto, importe}` (nunca como artículo).
2. Empareja cada línea detectada contra `lineas_pedido` del pedido por similitud de nombre normalizado (`emparejar`/`similitud`, umbral 0.4, greedy 1-a-1) — **nunca crea líneas nuevas**; lo que no encaja se guarda aparte (`sin_match`) solo a título informativo.
...
**Revisión humana obligatoria:** `abrirModalRevisionExtraccion` (`js/pedidos-render.js`) solo muestra una tabla comparativa con botones que **precargan** los modales de siempre — 📥 `openModalRecepcion(lineaId, pedidoId, cantidadSugerida)`, ✏️ `openModalEditarLinea(lineaId, pedidoId, precioSugerido)`, y para el cargo extra `openModalGastoExtra(pedidoId, conceptoSugerido, importeSugerido)` — con el valor detectado ya escrito en el campo. El Gestor sigue pulsando Guardar él mismo en el modal de siempre; la Edge Function de extracción no escribe nunca `cantidad_recibida`/`precio_unitario`/gasto extra por su cuenta. Decisión de diseño (no automatizar): un fallo de lectura de la IA nunca se cuela directo en stock o precios sin que alguien lo mire primero.
