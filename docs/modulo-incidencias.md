...
2. **Planificar** (`abrirPlanificacion` → `guardarPlanificacion`) → crea `Intervencion` con `Estado='Planificada'`; la incidencia pasa a `En gestión` y apunta a esa intervención; el equipo pasa a `Revisión planificada`. `Fecha_Planificada` es **opcional** — si aún no se sabe cuándo, se deja en blanco (se muestra "Por concretar" en badges/cards) y se añade más tarde reabriendo la planificación. El campo "Tareas ya previstas" (una por línea) crea tareas `Pendiente` en esa misma intervención sin esperar a la visita.
