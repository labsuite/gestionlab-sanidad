...
- **Solo se muestran/importan asignaciones profesor×módulo con laboratorio asignado en Sanidad CMA** (decisión de la usuaria; endurecido 2026-09-01): las filas con `laboratorio` vacío se descartan ya en `_cargarPreviewImportarProfesores` (filtro sobre `_previewProfesoresCMA`), antes de construir el checklist del paso 1. El contador de descartadas (`_profesoresSinLabDescartados`) se muestra como aviso en el paso 1 para que no desaparezcan en silencio. El guard equivalente del paso 2 (`sinLab` / `_profesoresAImportar = filter(p => p.labs.length)`) se mantiene como defensa pero ya no debería dispararse.
