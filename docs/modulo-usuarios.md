...
- **Solo se muestran/importan asignaciones profesor×módulo cuyo laboratorio tiene equipos en GestionLab** (decisión de la usuaria; endurecido 2026-09-01):
  ...
  - **Filtro en cliente (`_cargarPreviewImportarProfesores`):** de `laboratorio` se extraen los nº de 3 cifras (`/\d{3}/g`), se intersecan con los labs que tienen algún equipo en `DATA.equipos` (`_extraerLabDeUbicacion` sobre `Ubicacion`) y el resultado se guarda en `p.labsValidos`. Una fila solo pasa a `_previewProfesoresCMA` si `labsValidos.length`. Así se descartan aulas teóricas (`"Aula 200-1 (202)"`) y labs sin equipamiento inventariado (p.ej. Afondamento en "Lab 201"). El contador de descartadas (`_profesoresSinLabDescartados`) se avisa en el paso 1. Paso 2 usa `p.labsValidos` directamente.
