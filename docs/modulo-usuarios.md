...
- **Si ningún módulo del profesor tiene laboratorio asignado en Sanidad CMA, no se importa** (decisión de la usuaria, 2026-08-22): antes se creaba igual sin marcarlo responsable de nada; ahora se excluye directamente de `_profesoresAImportar` y se muestra un aviso con su nombre/email explicando por qué, para que no desaparezca en silencio. Si de los seleccionados en el paso 1 ninguno tiene lab, se corta ahí con un toast de error y no se pasa al paso 2.
